...
common to sequence fewer than 100 16S rRNA gene sequences from several samples
...
per fragment to yield a full-length sequence from a representative single molecule [ref]. At a cost of approximately
...
however, the single pass error rate is approximately 15%
...
by the 454 and Illumina platforms but with the same quality. To test this
...
pooling were described previously [ref]. The SMRTbell adapters were ligated onto 
the PCR products and the libraries were sequenced at the University of Michigan
DNA Sequencing Core using the P4-C2 chemistry on a PacBio RS II SMRT DNA 
Sequencing System.
...
were incorporated into mothur v.1.30. Knowledge of the true sequences for the 
strains in the mock community allowed us to quantify error rates and to identify
and remove chimeric sequences. This was performed using the seq.error command
...
17 times. Contrary to previous reports [ref], these results indicate that
