...
Now we'd like to go ahead and classify all of our sequences using both the RDP and greengenes training sets. We'll also classify the region-trimmed version of the Mock community sequence data:
...
for REGION in v*
do
  mothur "#classify.seqs(fasta=$REGION/$REGION.trim.unique.good.filter.unique.precluster.pick.fasta-$REGION/HMP_MOCK.filter.fasta, reference=trainset9_032012.pds.fasta, taxonomy=trainset9_032012.pds_rdp.tax, processors=8);
			classify.seqs(fasta=$REGION/$REGION.trim.unique.good.filter.unique.precluster.pick.fasta-$REGION/HMP_MOCK.filter.fasta, reference=gg_13_5_99.fasta, taxonomy=gg_13_5_99.pds_gg.tax, processors=8)"
done
```
...
getDepths <- function(region, database, cutoff=80){
  taxFileName <- paste(region, "/", region, ".trim.unique.good.filter.unique.precluster.pick.", database, ".wang.taxonomy", sep="")
...
for(r in regions){
  ...
  d.rdp <- getDepths("v4", "pds_rdp")
  d.gg <- getDepths("v4", "pds_gg")
  
  
  write.table(cbind("rdp" = d.rdp, "gg" = d.gg), file=paste(r, "/", r, ".tax.compare", sep=""), quote=F)
}
````
