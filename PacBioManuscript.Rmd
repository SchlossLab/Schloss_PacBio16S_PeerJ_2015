...
for REGION in v*
do
    cut -f 2 $REGION/$REGION.mock*error.summary | sort | uniq > mock.ref.accnos
    mothur "#get.seqs(fasta=$REGION/HMP_MOCK.filter.fasta, accnos=mock.ref.accnos); dist.seqs(cutoff=0.15, output=lt); cluster(phylip=current); summary.single(label=0.03, calc=sobs)"
done
```
