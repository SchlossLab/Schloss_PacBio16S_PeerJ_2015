...
It was difficult to pinpoint a threshold minimum average quality score based on the error rates alone. Let's try calculating the average quality score for the entire read...
...
The distribution here looks a little tighter across the different regions with something of an inflection around 60%, which corresponds to an average quality score of 55 to 65 for the regions (except for V4 and V1-V9). Let's propose we use an average quality score threshold of 60 since it produces the most consistent fraction of reads retained and error rate across the different regions. If we do this we get the following error rates:
...
getErrorRateFromAveQ <- function(folder, threshold=60){
  ...
  rates <- composite[composite$minQ >= threshold,]
...
#v4     0.005741810     0.002282200    0.78698225
#v13    0.014883360     0.006557198    0.12305003
#v35    0.013968573     0.003762152    0.15115654
#v15    0.009367667     0.002781731    0.28850637
#v16    0.014569803     0.004622321    0.03128922
#v19    0.013413156     0.003587509    0.04088219



```

Here we see that we have reduced the error rate by 56 to 75% when we require the average quality score for the read to be at least 60. In addition, we notice that, in general, the fraction of the reads we keep decreases as we increase the length of the fragment.
