...
```{r, engine='bash'}
for REGION in v4 v35 v15
do
  mothur "#get.groups(fasta=v4.trim.unique.good.filter.unique.fasta, name=v4.trim.unique.good.filter.names, group=v4.good.groups, groups=mock1.v4-mock2.v4-mock3.v4);
          degap.seqs();
          seq.error(fasta=current, name=current, reference=HMP_MOCK.filter.fasta, aligned=F, processors=8)"
done
```

regions <- c("v4", "v15", "v35")

rep1 <- v4/v4.mock1.unique.error.summary
rep2 <- v4/v4.mock2.unique.error.summary
rep3 <- v4/v4.mock3.unique.error.summary
