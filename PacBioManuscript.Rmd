...
generateComposite <- function(folder){
  ...
  lost.total <- 1 - nrow(composite)/n.seqs
...
plotBarcodePrimerError <- function(folder){
  mismatches <- read.table(file=paste(folder, "/", folder, ".mismatches", sep=""), header=F, row.names=1, col.names=c("x", "barcode", "primer"))
	error <- read.table(file=paste(folder, "/", folder, ".mock.filter.error.summary", sep=""), header=T, row.names=1)
  
  total.mismatch <- mismatches$barcode + mismatches$primer
       
  error.mismatch <- aggregate(error$error, by=list(total.mismatch), function(x){c(quantile(x, probs=c(0.025, 0.5, 0.975)), length(x))})$x
	...
	stripchart(error$error~total.mismatch, vertical=T, method="jitter", pch=19, col="grey", ylab="", yaxt="n", ylim=c(0,0.33))
...
pdf(file="test.pdf", height=10, width=8)
par(mfrow=c(3,2))
...
getFragment_BPErrorRates <- function(folder, length){
  ...
  error <- read.table(file=paste(folder, "/", folder, ".mock.filter.error.summary", sep=""), header=T, row.names=1)
	...
	bc.primer <- mismatches$barcode + mismatches$primer
  return(cbind(error$error, bc.primer/bp.length))
}
...
#t = 124.9934, df = 61936, p-value < 2.2e-16
...
# Median :0.004065   Median :0.00000  
# Mean   :0.020127   Mean   :0.01859  
# 3rd Qu.:0.023346   3rd Qu.:0.02128  
# Max.   :1.000000   Max.   :0.21739  


```

The last two commands shows us that there is a significant correlation between the barcode/primer error rate and the error rate of the rest of the fragment (R=0.45; P<<0.001) and that the average error rates are comparable. Based on this result, it is clear taht we want to minimize the number of mismatches to the barcodes and primers.
...
plotLines <- function(folder){
  ...
  coverage <- read.table(file=paste(folder, "/", folder, ".ccs.coverage", sep=""), header=T, row.names=1)
	...
	error.by.depth <- aggregate(error$error, by=list(coverage$freq), function(x){c(mean=mean(x), quantile(x, prob=c(0.025, 0.975)), n=length(x))})
...
par(mar=c(5,5,0.5, 0.5))
plot(1, xlim=c(0,60), ylim=c(0,0.08), type="n", xlab="", ylab="", yaxt="n")
title(xlab="Coverage", ylab="Error rate (%)")
axis(2, at=seq(0,0.1, 0.02), label=seq(0,10, 2), las=2)
lapply(regions, plotLines)
legend(x=40, y=0.08, legend=pretty.region[regions], lty=1, lwd=2, col=clrs[regions])
dev.off()
...
errorAtCoverageCutoff <- function(folder){
  ...
  total.seqs <- nrow(error)
	orig.error <- mean(error$error)
	
  deep <- coverage$freq >= 10
	coverage <- coverage[deep,]
  error <- error[deep,]

  covered.seqs <- nrow(coverage)
...
#           orig       final      kept
#v4  0.009008476 0.003933203 0.7588753
#v15 0.020436277 0.014950212 0.6154854
#v19 0.028188489 0.021484882 0.5190563
#v35 0.028413909 0.018121912 0.5712087
#v13 0.033327642 0.024590742 0.6007752
#v16 0.036096867 0.025843854 0.4083612
...
errorMinQualityScore <- function(folder){
  ...
  binned <- aggregate(error$error, by=list(round(aveq$minQ)), mean)
...
fractionMinQualityScore <- function(folder){
  ...
  binned <- aggregate(error$error, by=list(round(aveq$minQ)), length)
...
It was difficult to pinpoint a threshold minimum average quality score based on the error rates alone as each of the curves resembled a hockey stick. When we calculated the fraction of reads remaining for different thresholds, we observed a slight inflection in the curve at a minimum average quality score near 55. Let's see what happens when we look at the aggregate error rate of those reads with at least the threshold minimum average quality score
...
aggregateErrorMinQualityScore <- function(folder){
  ...
  remaining.error <- rep(0, max(aveq$minQ)+1)
  qscores <- 0:max(aveq$minQ)

  remaining.error <- unlist(lapply(qscores, function(x){mean(error[aveq$minQ >= x,"error"])}))
  names(remaining.error) <- qscores
  
  points(x=10:70, remaining.error[10:70], type="l", col=clrs[folder], lwd=2)
}

pdf(file="remainingError.pdf")
par(mar=c(5,5,0.5, 0.5))
plot(1, xlim=c(10,75), ylim=c(0,0.04), type="n", yaxt="n", xlab="", ylab="")
lapply(regions, aggregateErrorMinQualityScore)
title(xlab="Minimum average quality score\nwithin a 50-nt window across the full sequence", ylab="Error rate of sequences above threshold (%)")
axis(2, las=2, at=seq(0,0.04,0.01), label=seq(0,4,1))
legend(x=10, y=0.30, legend=pretty.region[regions], lty=1, lwd=2, col=clrs[regions])
dev.off()

```

We still don't see much of a break point. Let's try calculating the average quality score for the entire read...
...
errorAveQualityScore <- function(folder){
  ...
  binned <- aggregate(error$error, by=list(round(aveq$aveQ)), mean)
  binned <- binned[binned$Group.1 <= 70, ]
  points(binned$Group.1, binned$x, type="l", col=clrs[folder], lwd=2)
}
...
fractionAveQualityScore <- function(folder){
  ...
  binned <- aggregate(error$error, by=list(round(aveq$aveQ)), length)
...
Meh. Nothing much there. Let's propose we use an average quality score threshold of 60 since it produces the most consistent fraction of reads retained and error rate across the different regions. If we do this we get the following error rates:
...
getErrorRateFromAveQ <- function(folder, threshold=60){
  ...
  rates <- error[aveq$minQ >= threshold,]
  
  original.error <- mean(error[,"error"])
  remaining.error <- mean(rates[,"error"])
  fraction.kept <- nrow(rates)/nrow(error)
...
#v4     0.009008476     0.003129422     0.7897945
#v13    0.033327642     0.019663989     0.5451635
#v35    0.028413909     0.006071157     0.4590979
#v15    0.020436277     0.004383447     0.4343025
#v16    0.036096867     0.014667380     0.3133779
#v19    0.028188489     0.006732380     0.1955123

```

Here we see that we have reduced the error rate by 21 to 59% when we require the average quality score for the read to be at least 60. In addition, we notice that the fraction of the reads we keep decreases as we increase the length of the fragment.
...
|-------------------------------------------------------------------------------------------------------------------------------------------|
|v13 | 990		| 768		| 1022		| 693		| 711		| 722		| 410		| 991		| 325		| 756		| 549	| 799	|
|v15 | 519		| 652		| 1302		| 1337		| 1702		| 1491		| 807		| 606		| 1227		| 1167		| 1349	| 1393	|
|v16 | 442		| 386		| 530		| 204		| 197		| 198		| 134		| 404		| 234		| 294		| 243	| 180	|
|v19 | 132		| 46		| 105		| 360		| 262		| 290		| 230		| 94		| 87		| 319		| 375	| 591	|
|v35 | 982		| 876		| 804		| 831		| 1108		| 1089		| 447		| 444		| 298		| 538		| 295	| 917	|
|v4	 | 615		| 902		| 568		| 3699		| 4076		| 4828		| 1512		| 972		| 1167		| 722		| 2698	| 1301	|
...
|-------------------------------------
|v13  | 2780  | 2126   | 1726  | 2104
|v15  | 2473  | 4530   | 2640  | 3909
|v16  | 1358  | 599    | 772   | 717
|v19  | 283   | 912    | 411   | 1285
|v35  | 2662  | 3028   | 1189  | 1750
|v4   | 2085  | 12603  | 3651  | 4721

Let's sub-sample everything to 283 reads per sample

```{r engine="bash"}
for REGION in v*
do
    mothur "#summary.single(shared=$REGION/$REGION.trim.unique.good.filter.unique.precluster.pick.an.merge.shared, calc=sobs-coverage, subsample=283, iters=1000)"
done
```
...
do
  ...
  cat $REGION/$REGION.mock?.precluster.names > $REGION/$REGION.precluster.names
  mothur "#remove.seqs(fasta=$REGION/$REGION.mock.precluster.fasta, name=$REGION/$REGION.precluster.names, accnos=$REGION/$REGION.extra.chimeras);
...
do
  ...
  mothur "#classify.seqs(fasta=$REGION/$REGION.mock.precluster.fasta-$REGION/HMP_MOCK.filter.fasta, reference=trainset9_032012.pds.fasta, taxonomy=trainset9_032012.pds_rdp.tax, processors=8);
	         classify.seqs(fasta=$REGION/$REGION.mock.precluster.fasta, reference=gg_13_5_99.fasta, taxonomy=gg_13_5_99.pds_gg.tax, processors=8)"
done
